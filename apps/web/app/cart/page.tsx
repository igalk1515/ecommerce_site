      ...
      {cart.items?.map((item: any) => (
        <div key={item.id} className="card flex justify-between">
          ...
          <div className="flex gap-2 items-center">
            <input type="number" min={1} className="border rounded p-1 w-16" value={item.quantity} onChange={async (e) => { await api(`/cart/items/${item.id}`, { method: "PATCH", body: JSON.stringify({ quantity: Number(e.target.value) }) }); load(); }} />
            <button onClick={async () => { await api(`/cart/items/${item.id}`, { method: "DELETE" }); load(); }}>הסר</button>
          </div>
        </div>
      ))}
